...
const AdminPage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log('Adding vehicle with data:', formValues);
  ...
  const handleUpdate = async (e) => {
    ...
    formData.append('years', formValues.years);
    formData.append('pricePerDay', formValues.price);
    formData.append('available', formValues.available === 'true' ? true : false); // Assurez-vous que c'est un booléen
    formData.append('image', formValues.file);
    try {
  ...
  const handleEditClick = (vehicle) => {
    ...
    setFormValues({
      ...
      available: vehicle.available ? 'true' : 'false', // Convertir en string pour l'input
      file: null,
    });
    setIsEditing(true);
  };
